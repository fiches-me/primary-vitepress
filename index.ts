...
declare global {
  interface Window {
    __checkboxHandlerInitialized?: boolean;
  }
}
...
function processCheckboxes(): void {
...
function parseCustomCheckboxes(): void {
  ...
  listItems.forEach((li) => {
    ...
    if (match) {
      ...
      const existingCheckbox = li.querySelector<HTMLInputElement>(
        'input[type="checkbox"]',
      );
      if (existingCheckbox) {
        ...
        if (taskChar !== " " && taskChar !== "x" && taskChar !== "X") {
          ...
          existingCheckbox.dataset.task = taskChar;
...
function initCheckboxes(): void {
  ...
  const clickHandler = (e: MouseEvent) => {
    ...
    const listItem =
      (target.closest("li.task-list-item") as HTMLElement | null) ||
      (target.closest("li") as HTMLElement | null);
    if (!listItem) return;
